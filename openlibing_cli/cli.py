...
def resolve_refresh_token(config):
    env = os.environ.get("OPENLIBING_REFRESH_TOKEN")
    if env:
        return env
...
def make_api(args, config):
    ...
    def on_tokens(session_id, refresh_token):
    ...
    return ResourceManagerAPI(
        ENVIRONMENTS[env],
        resolve_session_id(args, config),
        env,
        refresh_token=resolve_refresh_token(config),
        on_tokens=on_tokens,
    )
...
def print_human(info):
    ...
    print()
    if info["useProxy"] and info.get("targetIp"):
        proxy = build_proxy_command(info)
        ...
        print(
            ...
            f"-o ProxyCommand={shlex.quote(proxy)} "
            f"-i {shlex.quote(info['identityFile'])} "
...
def build_proxy_command(info):
    jump_port = info.get("jumpPort") or 22
    return (
        "ssh "
        "-o StrictHostKeyChecking=no "
        "-o UserKnownHostsFile=/dev/null "
...
def cmd_login(args):
    ...
    update = {"session_id": ticket, "refresh_token": ""}
    if user_id:
    ...
    refresh_note = "(not fetched)"
    try:
        api = make_api(args, config)
        refresh_data = api.get_refresh_token()
        refresh = refresh_data.get("data")
        if refresh:
...
def cmd_ssh_config(args):
    ...
    if out["useProxy"] and out.get("targetIp"):
        jump_alias = re.sub(r"[^a-zA-Z0-9._-]", "-", f"openlibing-jump-{out['jumpIp']}-{out['jumpPort'] or 22}")
        block = (
            f"# Generated by openlibing-cli for {out['devEnvId']}\n"
            f"Host {jump_alias}\n"
            f"    HostName {out['jumpIp']}\n"
            f"    Port {out['jumpPort'] or 22}\n"
            f"    User jump\n"
...
def cmd_refresh(args):
    ...
    api = make_api(args, config)
    try:
        refresh = resolve_refresh_token(config)
        if not refresh:
        ...
        print(f"✗ {e}", file=sys.stderr)
        raise SystemExit(1)
    config.update({"session_id": new_session, "refresh_token": refresh})
...
def cmd_whoami(args):
    ...
    if refresh:
        print(f"refresh:    {refresh[:8]}…{refresh[-6:]}  ({len(refresh)} chars)")
    else:
        print("refresh:    (unset)")
